...
class SelectServerEditBox:
    ...
    def display(self):
        ...
        self.server_selected = tk.StringVar(top)
        tk.Label(top, text='Server',width=15, background='light gray',relief=tk.GROOVE).grid(row=0,column=0,pady=5,padx=5)
        ...
        b.grid(pady=5, padx=5, column=1, row=1, sticky="WE")
...
class ServerEntryBox:
    ...
    def display(self, server_id=None):            
        ...
        if server_id:
            ...
            self.server = self.servers_file.get_server_byname(server_id)
        else:
